...
> 本文档依据项目当前实际实现重写，覆盖后端全部已上线模块、Python SDK 与前端控制台。
...
```
启动:  init_tracing → start_lifecycle_scheduler → event_bus.start → start_webhook_worker
关闭:  stop_webhook_worker → event_bus.stop → stop_lifecycle_scheduler
```
...
- **融合策略**：
  ...
  - 支持结果 **重排序（rerank）**。
- **接口**：`hybrid-search`、`bm25`、`rerank`、`config`（GET/PUT）、`rebuild-index`（重建索引）。
...
- **LLM 后端**：`llm_backend_service.py` 封装 DeepSeek 等模型调用；`context_compressor.py` 负责超长上下文压缩（指标 `context_compression_seconds`）。
...
### 11.1 组织 / 工作区 / 成员（`workspace_service.py` · `/api/v1/workspaces`）
...
### 12.1 概述（`sdk-python`，包名 `agent_memory`）
...
| 构建 | Vite |
| 代码检查 | oxlint |
| 状态/服务 | stores + services（API 客户端） |
| 页面 | 记忆变量/表/片段、图谱、混合搜索、生命周期、可观测仪表盘、Agent Chat、会话、系统等 |
...
| hybrid_search | `/api/v1` (`/memory/hybrid-search`) | 混合搜索 |
...
| system_integration | `/api/v1/system` | 系统集成 |
| workspace | `/api/v1/workspaces` | 多租户工作区 |
| webhooks | `/api/v1/webhooks` | Webhook |
| events | `/api/v1/events` | 事件流 |

> 系统同时暴露：`/metrics`（Prometheus，`ENABLE_METRICS` 时）与 `/docs`（Swagger UI）。各模块详细端点见第 4–12 节。
...
├── api/                   # 19 个 Router（见第 17 节）
├── services/              # 26 个 Service（业务编排）
