...
> **文档版本**：v3.0  ·  **最后更新**：2026-07-02
> 本文档依据项目当前实际实现重写，覆盖后端全部已上线模块（含 P1 中期改进：混合搜索缓存/分页/分析、LLM 断路器/重试队列、前端虚拟列表/Web Worker/PWA）、Python SDK 与前端控制台。
...
- **贡献度分析（P1）**：
  - `analyze_search(user_id, query, workspace_id)` 复用检索流程，返回：
    ...
    - 权重敏感度（对 `alpha/beta/gamma/delta` 各 ±0.1 重算 top-k，报告排名变化数）。
