...
| 能力域 | 模块 | 说明 |
|--------|------|------|
| 🧠 记忆存储 | 记忆变量 | 轻量级 KV 存储（如 `user_name: "鑫海"`），支持加密与 TTL；数据库 best-effort 双写备份（G3）+ 恢复/统计/清理端点 |
...
| 🔍 智能检索 | 自动召回 | 基于语义相似度的记忆检索与上下文注入；P0 多会话聚合 + 时间感知召回 + 知识更新检测；P1 会话分解索引 + 多键索引 + 时间感知查询扩展；支持 Agent 作用域过滤；`/recall/auto` 三层分层召回（Profile → 语义 → 实体展开，token 预算控制） |
| 🔍 智能检索 | 混合搜索 | 向量 + BM25 + 时间 + 重要度的 RRF 融合检索与重排序；Redis 查询缓存、结果分页、贡献度分析 |
| 🧬 嵌入 | Embedding 抽象 | 可插拔嵌入后端：Chroma 内置默认模型 / 本地 sentence-transformers（BGE 等），集合后缀隔离 + 向量重建脚本；启动预热消除首次语义召回冷启动（G1），查询嵌入失败自动回退 Chroma 内置嵌入 |
...
| 🔌 集成 | MCP Server | Model Context Protocol 标准协议，10 个工具（7 记忆管理 + 3 知识图谱），支持 stdio/streamable_http/SSE；挂载宿主应用时 stateless_http 无状态会话 + lifespan 并入（防 "Connection closed"） |
| 🧰 集成 | Python SDK | HTTP / 嵌入两种模式，LangChain 集成；异步客户端支持上下文管理与记忆/图谱/生命周期便捷方法（0.1.1） |
...
启动:  init_tracing → start_lifecycle_scheduler → event_bus.start
       ...
       → asyncio.create_task(warmup_embedding_models)   # G1 后台预热，不阻塞启动
关闭:  cancel warmup_task（若未完成）→ stop_retry_worker
       → stop_webhook_worker → event_bus.stop → stop_lifecycle_scheduler
```
...
- `warmup_embedding_models`：Embedding 启动预热（G1，详见 3.13），后台任务执行、失败绝不阻断启动；shutdown 时未完成则取消。
- **MCP 挂载时的 lifespan 并入**：Starlette 对 `mount()` 挂载的子应用不触发 lifespan 事件，会导致 MCP `StreamableHTTPSessionManager` 的 task group 永不启动（客户端表现为 "Connection closed / Task group is not initialized"）。`mount_to_app()` 将 MCP session manager 的运行周期包装进宿主应用的 `lifespan_context`，随宿主启停（详见第 17 节）。
...
| 嵌入预热 | `EMBEDDING_WARMUP_ENABLED` / `EMBEDDING_WARMUP_TIMEOUT` / `EMBEDDING_WARMUP_RETRIES` | `True` / `30.0` / `2` | 启动预热开关 / 单次尝试超时（秒）/ 失败重试次数（指数退避）（G1） |
| 嵌入预热 | `READINESS_WAIT_FOR_WARMUP` | `True` | `/health/ready` 是否等待预热完成（预热 failed 不阻断，仅降级运行） |
...
| 变量备份 | `VARIABLES_DB_BACKUP_ENABLED` | `True` | 记忆变量数据库双写备份开关（G3）；`False` 时双写与恢复全部短路 |
...
### 3.13 Embedding 启动预热与查询降级（G1 · `app/core/embedding_warmup.py`）

embedding 模型是**懒加载**的（local 模式首次 `embed_query` 加载 sentence-transformers 权重；default 模式由 Chroma 首次查询加载内置 all-MiniLM-L6-v2），导致首次语义召回延迟 5–8 秒，易超出调用方超时预算。G1 在 startup 阶段后台预热并提供 readiness 门控：

| 组件 | 说明 |
|------|------|
| `warmup_embedding_models()` | 后台预热入口：local 模式用中英文预热文本触发 `embed_query`；default 模式对 Chroma 做轻量查询。线程 + 超时保护，失败按 `EMBEDDING_WARMUP_RETRIES` 指数退避重试；**任何情况不抛异常，预热失败绝不阻断服务启动** |
| `get_warmup_status()` | 返回 `status`（`pending` \| `warming_up` \| `completed` \| `failed`）与起止时间，供 readiness 检查读取 |
| `/health/ready` 门控 | 新增 `embedding_warmup` 检查项：`READINESS_WAIT_FOR_WARMUP=True` 时预热未完成（pending/warming_up）返回 503 摘流；**failed 仅记录为降级状态、不置 not_ready**（懒加载兜底，避免实例永久摘流） |
| `chromadb_client.search_embeddings` 降级 | 查询嵌入计算失败时回退 Chroma 内置嵌入（稳定日志标记 `[embedding_fallback]`），异常不再冒泡阻断检索 |
| lifespan 集成 | `asyncio.create_task` 后台预热不阻塞启动 `yield`；shutdown 时未完成则 cancel |

**部署配套**：`k8s/deployment.yaml` readinessProbe `initialDelaySeconds` 调整为 30 以匹配预热窗口（ConfigMap 显式 `READINESS_WAIT_FOR_WARMUP="true"`）；`docker-compose.prod.yml` healthcheck 走 `/health/live`，不受预热门控影响。
...
- **特性**：
  ...
  - **数据库备份（G3）**：Redis 为主存储，写成功后 best-effort 双写镜像到 `memory_variables_backup` 表（失败仅 WARNING，绝不影响 Redis 主路径）；`workspace_id`/`session_id` 用哨兵值归一（空→0 / 空→''）规避 SQLite UNIQUE 中 NULL 不互斥导致的 upsert 不幂等问题。`VARIABLES_DB_BACKUP_ENABLED=False` 时双写与恢复全部短路。
- **接口**：变量的增删改查、批量列举，以及备份管理端点：

| 端点 | 方法 | 说明 |
|------|------|------|
| `/memory/variables/restore-backup` | POST | 从数据库备份恢复变量到 Redis（**仅补缺失 key，不覆盖现值**） |
| `/memory/variables/backup-stats` | GET | 备份统计（总行数 / 未过期行数 / 开关状态） |
| `/memory/variables/backup-purge` | POST | 物理清理备份表中已过期行（防止备份表无限增长） |

> 备份管理端点为静态路由，注册在 `/variables/{key}` 通配路由之前。
...
- **三层分层召回（`POST /memory/recall/auto`）**：面向 Agent 上下文装配的分层输出，请求体 `{query, token_budget=4000, layers?}`，返回：

| 层 | 内容 | 说明 |
|------|------|------|
| `profile` | KV 变量 + 高重要度片段（importance ≥ 0.6，前 5 条、单条截断 200 字） | 用户画像层，优先注入 |
| `semantic` | 全部语义片段（前 10 条内容拼接） | 语义相关层 |
| `entity_expansion` | 召回上下文摘要（前 1000 字符） | 实体展开层 |

  另返回 `total_tokens` 估算（总字符数 // 2）。`token_budget` 驱动 `top_k = max(token_budget // 800, 3)`，实现预算自适应检索量。
...
| `streamable_http` | HTTP 流式传输（挂载宿主应用时启用 `stateless_http`，每请求独立会话） | 远程服务调用 |
| `SSE` | Server-Sent Events | 实时事件推送 |

**挂载可靠性（streamable_http）**：被 `mount()` 到宿主 FastAPI 应用时采用两项加固，防止 "Connection closed / Session not found / Task group is not initialized" 类故障：

1. **`stateless_http=True` + `streamable_http_path="/"`**：每请求独立会话、不依赖 session ID，服务重启/重载后客户端无需重新握手；端点即挂载路径本身（避免 `/mcp/mcp` 双重前缀）。
2. **lifespan 并入**：Starlette 对 `mount()` 挂载的子应用不触发 lifespan 事件，`mount_to_app()` 将 `mcp.session_manager` 的运行周期包装进宿主应用的 `lifespan_context`，保证 session manager 的 task group 随宿主启停（见 2.3 节）。

### 17.3 工具清单（10 个，以运行期 `tools/list` 为准）
...
| 记忆管理 | `memory_get_context` | 获取记忆上下文 |
| 记忆管理 | `memory_create_table` | 创建结构化记忆表 |
| 记忆管理 | `memory_add_record` | 向记忆表添加记录 |
| 知识图谱 | `graph_add_entity` | 添加实体 |
| 知识图谱 | `graph_search_entities` | 实体搜索 |
| 知识图谱 | `graph_query_neighbors` | 邻居查询 |
...
**异步客户端（`async_client.py`，0.1.1 增强）**：

- 支持 `async with` 上下文管理与底层 `request()` 公共透传。
- 记忆便捷方法：`remember` / `recall_context` / `forget` / `get_variable` / `list_variables` / `search` / `remember_fragment`。
- 结构化：`create_table` / `list_tables` / `add_record` / `query_records` / `update_record`。
- 图谱与治理：`graph_query` / `graph_ingest` / `run_lifecycle_maintenance` / `detect_memory_conflicts` / `extract_and_save`。
- HTTP 传输层错误处理增强（状态码→异常类型映射）。
...
- **生产**：SQLite/PostgreSQL + Chroma + Redis（EventBus=redis）+ OTLP Collector + Prometheus 抓取 `/metrics`。注意 PG 下 BM25 降级为 LIKE/ILIKE（见 3.2）；Milvus/Qdrant 尚不可用（见 3.3）。健康探针分 `/health/live`（存活）与 `/health/ready`（就绪，含向量库与 Embedding 预热门控，见 3.13）。
...
| memory_variables | `/api/v1/memory` | 记忆变量（含 G3 备份管理端点 restore-backup / backup-stats / backup-purge） |
...
| auto_recall | `/api/v1/memory/recall` | 自动召回（含 `/recall/auto` 三层分层召回） |
...
系统共 **39 张核心表**（SQLite / PostgreSQL），按域划分：
...
| 表 | 说明 |
|----|------|
| `memory_variables` | KV 记忆变量（Redis 主存储） |
| `memory_variables_backup` | 记忆变量数据库备份镜像（G3：best-effort 双写；`UNIQUE(user_id, workspace_id, session_id, key)`，空值哨兵归一；含 `ttl_seconds`/`expires_at`） |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v6.2 | 2026-08-24 | 依据 7 月底以来实际提交更新：Embedding 启动预热（G1：`embedding_warmup.py` 后台预热 + `/health/ready` 门控 + `search_embeddings` 查询降级回退 + k8s/docker 部署配套，新增 3.13 节与 4 个配置项）；记忆变量数据库 best-effort 双写备份（G3：`memory_variables_backup` 表 + restore/backup-stats/backup-purge 三端点 + `VARIABLES_DB_BACKUP_ENABLED` 开关，4.1/24/25 节更新，38→39 张表）；三层分层召回 `POST /recall/auto`（Profile → 语义 → 实体展开，token_budget 自适应，5.1 节）；MCP Server 挂载可靠性加固（stateless_http + streamable_http_path="/" + lifespan 并入宿主应用，17.2/2.3 节）；Python SDK 异步客户端能力增强（便捷方法 + 错误处理，0.1.1） |
