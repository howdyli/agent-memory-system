...
> **文档版本**：v6.0  ·  **最后更新**：2026-07-27
> 本文档依据项目当前实际实现重写，覆盖后端全部已上线模块。v6.0 新增：Embedding Provider 可插拔抽象（default/local + 集合后缀隔离 + 向量重建脚本）、睡眠期记忆巩固（R-11）、Agent 注册与跨 Agent 记忆作用域（R-12）、多模态记忆（R-13）、程序记忆（R-14）、图谱社区检测（R-15，Louvain）、LoCoMo 基准（1540 题四维度）与 A/B 嵌入评测脚本、记忆 Playground、维护任务扩展为 7 步、半衰期类型扩展（multimodal/procedure）。
...
- **向量重建**：切换嵌入模型后运行 `scripts/reindex_vectors.py`，将现有记忆片段/图谱实体重新嵌入写入新后缀集合。
...
| 页面 | 记忆变量/表/片段、图谱、混合搜索、生命周期、可观测仪表盘、Playground、Agent Chat（虚拟列表）、会话、系统等 |
...
- **生产**：SQLite/PostgreSQL + Chroma/Milvus/Qdrant + Redis（EventBus=redis）+ OTLP Collector + Prometheus 抓取 `/metrics`。
...
- **嵌入切换**：变更 `EMBEDDING_PROVIDER`/`EMBEDDING_MODEL` 后必须运行 `scripts/reindex_vectors.py`，否则新旧向量分处不同集合。
...
├── services/              # 38+ 个 Service（含 consolidation / agent_registry / multimodal
│                          # tool_trace / procedure_extraction / graph_community）
